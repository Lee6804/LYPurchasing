...
@interface ClassifyVC ()<SPPageMenuDelegate,UIScrollViewDelegate>
...
-(ClassifyNavView *)navView{
    if (!_navView) {
        _navView = [[ClassifyNavView alloc] initWithFrame:CGRectMake(0, 0, MainWidth, 44)];
    }
    return _navView;
}
...
-(void)initVC{
    ...
    self.dataArr = @[@"分类",@"品牌"];
    // trackerStyle:跟踪器的样式
    SPPageMenu *pageMenu = [SPPageMenu pageMenuWithFrame:CGRectMake(MainWidth/2 -70, 0, 140, pageMenuH) trackerStyle:SPPageMenuTrackerStyleLineAttachment];
    ...
    NSArray *controllerClassNames = [NSArray arrayWithObjects:@"ClassifySubVC",@"ClassifyBrandVC", nil];
